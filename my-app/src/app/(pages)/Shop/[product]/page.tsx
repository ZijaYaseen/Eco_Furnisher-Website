export const revalidate = 60; // seconds

import ProductDetailClient from "@/components/ClientDetailProductPage";
import { client } from "@/sanity/lib/client";
import Link from "next/link";

// Query Constants
const staticPathsQuery = `*[_type == "product"]{ "slug": slug.current }`;

const productMetaQuery = `*[_type == "product" && slug.current == $product][0]{
  productNameEn,
  seo {
    metaTitle,
    metaDescription
  },
  shortDescription
}`;

const productDetailQuery = `*[_type == "product" && slug.current == $product][0]{
  slug,             
  _id,
  productNameEn,
  productSku,
  "imageSet": productImageSet[],
  rating,
  shortDescription,
  description,
  categoryId,
  CategoryName,
  packingWeight,
  shippingCharge, 
  inventory,
  tags,
  seo {
    metaTitle,
    metaDescription,
    metaKeywords
  },
  variants[]{    
    vid,
    variantSellPrice,
    variantSugSellPrice,
    variantActualSellPrice,  
    discountPercentage,
    colors{               
      colorName,
      colorCode
    },
    variantImage     
  }
}`;


// Generate static routes using product slug
export async function generateStaticParams() {
  const products: { slug: string }[] = await client.fetch(staticPathsQuery);
  return products.map((prod) => ({ product: prod.slug }));
}

// Set metadata for SEO
export async function generateMetadata({ params }: { params: { product: string } }) {
  const product = await client.fetch(productMetaQuery, { product: params.product });
  if (!product) {
    return { title: "Product Not Found" };
  }
  return {
    title: product.seo?.metaTitle || product.productNameEn,
    description: product.seo?.metaDescription || product.shortDescription,
  };
}

// Fetch product data and render the client component
export default async function ProductPage({ params }: { params: { product: string } }) {
  const product = await client.fetch(productDetailQuery, { product: params.product }, { cache: "no-store" });
  
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-center">
        <h1 className="text-2xl font-bold text-red-600">Product Not Found</h1>
        <p className="text-gray-500 mt-2">The product you are looking for does not exist.</p>
        <Link href="/">
          <button className="mt-4 px-6 py-2 bg-gray-950 text-white rounded-md hover:bg-gray-800">
            Go to Home Page
          </button>
        </Link>
      </div>
    );
  }

  return <ProductDetailClient product={product} />;
}
